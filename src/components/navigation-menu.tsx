...
import { GridCard } from '_/components/grid-card';
...
function NavSmallItem({
	...
	return (
		<NavigationMenuLink
			className={cn(
				'relative h-max flex items-center  px-2 py-2',
				className,
			)}
			{...props}
		>
			<div className='group/links w-full flex items-center ' >
				{item.icon && <item.icon />}
				<p className="text-sm">{item.title}</p>
				<div className="relative ml-auto flex h-full w-4 items-center">
					<ArrowRightIcon className="size-4 -translate-x-2 opacity-0! transition-all group-hover/links:translate-x-0 group-hover/links:opacity-100!" />
				</div>
			</div>
		</NavigationMenuLink>
	);
